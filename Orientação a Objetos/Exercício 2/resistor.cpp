...
string resistor::get_unidade_fisica(void) {
    return "Ohms";
}
...
resistor resistor::operator+(resistor &outro_R) {
    resistor req;     //!< resistor equivalente da associação em série
    req.nominal = get_R_com_unidade() + outro_R.get_R_com_unidade();
    return req;
}

resistor resistor::operator/(resistor &outro_R) {
    resistor req;     //!< resistor equivalente da associação em paralelo
    req.nominal = (get_R_com_unidade() * outro_R.get_R_com_unidade()) / (get_R_com_unidade() + outro_R.get_R_com_unidade());
    return req;
}

ostream& operator << (ostream &saida, resistor &r1) {
    saida << r1.get_R_com_unidade() << " " << r1.get_unidade_fisica();
    return saida;
}
